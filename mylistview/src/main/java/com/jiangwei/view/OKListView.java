...
import com.oklistview.luffy.mylistview.R;

/**
 * author: jiangwei18 on 17/4/4 23:00 email: jiangwei18@baidu.com Hi: jwill金牛
 */

public class OKListView extends ListView implements AbsListView.OnScrollListener {
    private static final String TAG = "RefreshListView";
    private int firstVisibleItemPosition; // 屏幕显示在第一个的item的索引
    private int downY; // 按下时y轴的偏移量
    private int upY; // 抬起时y轴的偏移量
    ...
    private boolean isRefreshing = false; // 是否正在更新中
    private boolean isPulling = false; // 是否正在下拉当中
    // hideHeader的值动画
    ...
    public OKListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initHeaderView();
        initFooterView();
    ...
    private void initHeaderView() {
        ...
        // 设置最后刷新时间
        tvLastUpdateTime.setText(getContext().getString(R.string.refresh_last_time, getLastUpdateTime()));
    ...
    private void initAnimation() {
        upAnimation =
                new RotateAnimation(0f, -180f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        upAnimation.setDuration(500);
        upAnimation.setFillAfter(true); // 动画结束后, 停留在结束的位置上

        downAnimation =
                new RotateAnimation(-180f, -360f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (!isRefreshing && !isPulling && !isLoadingMore && mCanRefresh) {
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (firstVisibleItemPosition == 0 && -headerViewHeight < paddingTop) {
                        if (paddingTop > 0 && currentState == DOWN_PULL_REFRESH) { // 完全显示了.
                            Log.i(TAG, getContext().getString(R.string.release_refresh));
                            currentState = RELEASE_REFRESH;
                            refreshHeaderView();
                        } else if (paddingTop < 0 && currentState == RELEASE_REFRESH) { // 没有显示完全
                            Log.i(TAG, getContext().getString(R.string.pull_refresh));
                ...
                case MotionEvent.ACTION_UP:
                    // 判断当前的状态是松开刷新还是下拉刷新
                    upY = (int) ev.getY();
                    top = -headerViewHeight + (upY - downY) / 2;
                    if (currentState == RELEASE_REFRESH) {
                        Log.i(TAG, getContext().getString(R.string.refresh_data));
                        // 把头布局设置为完全显示状态
    ...
    private void refreshHeaderView() {
        switch (currentState) {
            case DOWN_PULL_REFRESH: // 下拉刷新状态
                tvState.setText(getContext().getString(R.string.pull_refresh));
                ivArrow.startAnimation(downAnimation); // 执行向下旋转
                break;
            case RELEASE_REFRESH: // 松开刷新状态
                tvState.setText(getContext().getString(R.string.release_refresh));
            ...
            case REFRESHING: // 正在刷新中状态
                ...
                tvState.setText(getContext().getString(R.string.refreshing));
                break;
            default:
                break;
        }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_FLING) {
            // 判断当前是否已经到了底部
            if (isFillWindow && !isRefreshing && isScrollToBottom) {
                isPulling = false;
            }

            if (isFillWindow && isScrollToBottom && !isLoadingMore && !isRefreshing && !isPulling && mCanLoadMore) {
                isPulling = false;
                isLoadingMore = true;
                // 当前到底部
                Log.i(TAG, getContext().getString(R.string.load_more));
     ...
     * 隐藏头布局
     */
    public void hideHeaderView(boolean isSuccess) {
        int paddingTop = 0;
        long time = ANIMATOR_TIME;
        if (isSuccess) {
            tvLastUpdateTime.setText(getContext().getString(R.string.refresh_last_time, getLastUpdateTime()));
        } else {
            paddingTop = top;
            float f = ((float) paddingTop / -headerViewHeight);
            f = 1.0f - f;
            time = (long) (f * ANIMATOR_TIME);
        }
        animator = ValueAnimator.ofInt(paddingTop, -headerViewHeight);
    ...
    private void hideHeadViewState() {
        isPulling = false;
        isRefreshing = false;
